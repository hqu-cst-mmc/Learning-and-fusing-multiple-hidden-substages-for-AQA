...
parser.add_argument("--lr_steps", default=[50, 60], type=int, nargs="+",
...
def main(options):
    ...
    # train_file = './data_files/training_idx.npy'
    # test_file = './data_files/testing_idx.npy'
    # # data_folder = '/media/donglijia/新加卷2/xiaodong_dataset/AQA-7/frame_cut/sync_diving_3m/'
    # data_folder = '/home/donglijia/dlj/diving-score-master/frames'
    # range_file = './data_files/tcn_time_point.npy'
    # label_file = './data_files/overall_scores.npy'
    ...
    if options.load:
        ...
        checkpoint = torch.load('./results[最终版]-0.77/checkpoint' + str(options.load) + '.tar')
            ...
            for epoch_i in range(0, options.epochs):
                ...
                for it, train_data in enumerate(train_loader, 0):
                    ...
                    #     # logging.info("loss at iteration {0}: {1}".format(it, loss.item()))
                        f = open('./results/result_ours_train(sync3).txt', mode='a')
                ...
                f = open('./results/train—loss(sync3).txt', mode='a')
                f.write('\n' + 'train loss at epoch' + str(epoch_i) + ':' + '\n')
                f.write(str(train_avg_loss))
                f.write('\n')

                f = open('./results/weights_of_5scores(sync3).txt', mode='a')
                ...
                with torch.no_grad():
                    ...
                    for it, test_data in enumerate(test_loader, 0):
                        ...
                        if it % 10 == 0:
                            # # print(train_output.data.cpu().numpy()[0][0], '-', labels_single.data.cpu().numpy()[0])
                            #     # logging.info("loss at iteration {0}: {1}".format(it, loss.item()))
                            f = open('./results/result_ours_test(sync3).txt', mode='a')
      ...
      with torch.no_grad():
          ...
          for test_times in range(0, 35):
                ...
                for i in range(len(labels_single.data.cpu().numpy())):
                    f = open('./results-rw1/res_for_test1.txt', mode='a')
                    f.write(
                ...
                logging.info("TEST TIME {0} ==> The corr is {1} , the MSE is {2}, the MDE is {3}, the time is {4}".format(test_times, rho, mse, mde, elapse_time))

                f = open('./results-rw1/metrics1.txt', mode='a')
                f.write('\n' + str(test_times) + '-' + 'rho:' + str(rho) + '-' + 'mse:' + str(mse) + '-' + 'med:' + str(mde) + '-' + 'time:' + str(elapse_time))
          ...
          logging.info(
              "Average corr is {0}, MSE is {1}, MDE is {2},Elapse time is{3}".format(
