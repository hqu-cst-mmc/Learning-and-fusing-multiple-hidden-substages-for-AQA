    ...
    if options.only_last_layer:
        ...
        if options.optimizer == "SGD":
            ...
            optimizer = eval("torch.optim." + options.optimizer)([{'params':model.parameters()},{'params':ttc_model.parameters()},
                                                                  ...
                                                                  {'params':ttc_for_con_model.parameters(),'lr':options.learning_rate*10},
                ...
                for it, test_data in enumerate(test_loader, 0):
                    ...
                    if times == 3 :
                        for i in range(len(labels_single.data.cpu().numpy())):
                            f = open('./result/res_for_test(totalScore).txt', mode='a')
                            f.write(
                            ...
                            f.write('\n' + 'avg_test_res(tt-score):' + '\n' + str(test_output_sum_avg[i]) + '-' + str(
                                labels_single.data.cpu().numpy()[i]))

                rho1, p_val1 = spearmanr(all_test_ouput_sub1, all_labels)
                rho2, p_val2 = spearmanr(all_test_ouput_sub2, all_labels)
                rho3, p_val2 = spearmanr(all_test_output_avg, all_labels)
                logging.info("The corr of subNet1 is {0},the corr of subNet2 is {1}, the corr of avg is {2}".format(
                    rho1, rho2, rho3))
                all_rho1 = np.append(all_rho1,rho1)
                all_rho2 = np.append(all_rho2,rho2)
                all_rho3 = np.append(all_rho3,rho3)
            rho_avg1 = np.average(all_rho1)
            rho_avg2 = np.average(all_rho2)
            rho_avg3 = np.average(all_rho3)
            logging.info("The avg corr of subNet1 is {0},the avg corr of subNet2 is {1}, the avg corr of avg is {2}".format(
                rho_avg1, rho_avg2, rho_avg3))
