...
def main(options):
    ...
    if options.load:
        ...
        checkpoint = torch.load('./results/checkpoint' + str(options.load) + '.tar')
        ...
        if options.allstage:
            for epoch_i in range(0, options.epochs):
                ...
                for it, train_data in enumerate(train_loader, 0):
                    ...
                    ttc_model.train()
                    ## [2,1]
                    train_output_sum = ttc_model(train_output_sum) * 30
                    ...
                    #     # logging.info("loss at iteration {0}: {1}".format(it, loss.item()))
                        f = open('./results/result_ours_train.txt', mode='a')
                        f.write('\n' + str(train_output_sum_final.data.cpu().numpy()[0][0]) + '-' + str(scores_single.data.cpu().numpy()[0]))
                        f.write('\n')
                ...
                f = open('./results/train—loss.txt', mode='a')
                ...
                writer1.add_scalar('diving-5stages-5scores-5fc', train_avg_loss, epoch_i + 1)
                ...
                with torch.no_grad():
                    ...
                    for it, test_data in enumerate(test_loader, 0):
                        ...
                        test_output_sum = torch.cat((vid_tensor_save[0], vid_tensor_save[1],
                                                      vid_tensor_save[2], vid_tensor_save[3], vid_tensor_save[4]), 1)
                        test_output_sum = ttc_model(test_output_sum) * 30
                        ...
                        if it % 10 == 0:
                            # # print(train_output.data.cpu().numpy()[0][0], '-', labels_single.data.cpu().numpy()[0])
                            #     # logging.info("loss at iteration {0}: {1}".format(it, loss.item()))
                            f = open('./results/result_ours_test.txt', mode='a')
                            f.write('\n' + str(test_output_sum_final.data.cpu().numpy()[0][0]) + '-' + str(
                ...
                writer2.add_scalar('diving-5stages-5scores-5fc', test_avg_loss, epoch_i + 1)
                if rho > options.stop:
                    break
#######################################################################################################
        ### loss可视化
      ...
      with torch.no_grad():
        ...
        for times in range(0,35):
              ...
              ttc_model.eval()
              weights_tscore = ttc_model.fc_ttc.weight.data.cpu().numpy()
              # f = open('./test-results/weights_of_5scores.txt', mode='a')
              # f.write('weights of 5scores:' + str(weights_tscore[0]))
              # f.write('\n')
              tfc_model1.eval()
              tfc_model2.eval()
              ...
              for it, test_data in enumerate(test_loader, 0):
                      ...
                      vid_tensor_save = []
                      for tcncover in range(1, 6):
                          vid_tensor, labels, diffs, scores, vid_name = test_data
                          if use_cuda:
                              vid_tensor_single, labels_single, diffs_single, scores_single = Variable(
                                  vid_tensor[tcncover - 1]).cuda(), Variable(
                                  labels[tcncover - 1]).cuda(), Variable(diffs[tcncover - 1]).cuda(), Variable(
                      ...
                      test_output_sum = torch.cat((vid_tensor_save[0], vid_tensor_save[1],
                                                   vid_tensor_save[2], vid_tensor_save[3], vid_tensor_save[4]), 1)
                      # f = open('./test-results/test_subscores.txt', mode='a')
                      # f.write('\n' + 'video name:' + str(vid_name))
                      # f.write('\n' + 'stage1:' + str(vid_tensor_save[0].data.cpu().numpy()[0][0]))
                      # f.write('\n' + 'stage2:' + str(vid_tensor_save[1].data.cpu().numpy()[0][0]))
                      # f.write('\n' + 'stage3:' + str(vid_tensor_save[2].data.cpu().numpy()[0][0]))
                      # f.write('\n' + 'stage4:' + str(vid_tensor_save[3].data.cpu().numpy()[0][0]))
                      # f.write('\n' + 'stage5:' + str(vid_tensor_save[4].data.cpu().numpy()[0][0]))
                      # f.write('\n')
                      test_output_sum = ttc_model(test_output_sum) * 30
                      loss = criterion(test_output_sum, labels_single)
                      test_output_sum_final = test_output_sum * diffs_single
                      ...
                      for i in range(len(labels_single.data.cpu().numpy())):
                            f = open('./result-rw1/res_for_test1.txt', mode='a')
                            f.write('\n' + str(test_output_sum_final[i].data.cpu().numpy()) + '-' + str(scores_single.data.cpu().numpy()[i]))
              rho, p_val = spearmanr(all_test_output, all_scores)
              n = len(all_scores)
