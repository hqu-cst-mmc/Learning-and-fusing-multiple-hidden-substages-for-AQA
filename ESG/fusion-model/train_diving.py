...
parser.add_argument("--stop", default=0.93, type=float,
...
def main(options):
    ...
    if options.test:
        ...
        writer1 = SummaryWriter('runs/exp/train1')
        writer2 = SummaryWriter('runs/exp/test1')
    ...
    if options.load:
        ...
        checkpoint = torch.load('./result-2loss-0.89/checkpoint' + str(options.load) + '.tar')
        ...
        if options.allstage:
            for epoch_i in range(0, options.epochs):
                ...
                for it, train_data in enumerate(train_loader, 0):
                    ...
                    ttc_model.train()
                    ## [2,1]
                    train_output_sum_subnet2 = ttc_model(train_output_sum_subnet2)*30
                    train_output_subnet2_final = train_output_sum_subnet2 * diffs_single
                    train_output_sum_subnet1 = ttc_for_con_model(train_output_sum_subnet1) * 30
                    train_output_subnet1_final = train_output_sum_subnet1 * diffs_single
                    ...
                    # 度量loss
                    ## if use 3 loss, uncomment this
                    loss3 = criterion(train_output_sum_subnet1,train_output_sum_subnet2 )
                    loss = loss1 + loss2 + loss3

                    train_output_sum_avg = ((train_output_subnet1_final + train_output_subnet2_final) / 2)

                    all_train_output_avg = np.append(all_train_output_avg,train_output_sum_avg.data.cpu().numpy()[:, 0])
                    all_train_output_sub1 = np.append(all_train_output_sub1, train_output_subnet1_final.data.cpu().numpy()[:, 0])
                    all_train_output_sub2 = np.append(all_train_output_sub2, train_output_subnet2_final.data.cpu().numpy()[:, 0])

                    all_scores = np.append(all_scores, scores_single.data.cpu().numpy())
                    if it % 100 == 0:
                            f = open('./result/result_ours_train.txt', mode='a')
                            f.write('\n' + 'sub1_train_res:' + str(train_output_subnet1_final.data.cpu().numpy()[0]) + '-' + str(
                                scores_single.data.cpu().numpy()[0]))
                            f.write('\n' + 'sub2_train_res:' + str(train_output_subnet2_final.data.cpu().numpy()[0]) + '-' + str(
                                scores_single.data.cpu().numpy()[0]))
                            f.write('\n' + 'avg_train_res:' + str(train_output_sum_avg[0]) + '-' + str(scores_single.data.cpu().numpy()[0]))
                            f.write('\n')
                ...
                rho1, p_val = spearmanr(all_train_output_sub1, all_scores)
                rho2, p_val = spearmanr(all_train_output_sub2, all_scores)
                rho3, p_val = spearmanr(all_train_output_avg, all_scores)
                ...
                with torch.no_grad():
                    ...
                    for it, test_data in enumerate(test_loader, 0):
                        ...
                        test_output_sum_subnet2 = ttc_model(test_output_sum_subnet2)*30
                        test_output_subnet2_final = test_output_sum_subnet2 *diffs_single
                        test_output_sum_subnet1 = ttc_for_con_model(test_output_sum_subnet1)*30
                        test_output_subnet1_final = test_output_sum_subnet1  *diffs_single
                        ...
                        test_output_sum_avg = ((test_output_subnet1_final + test_output_subnet2_final) / 2).data.cpu().numpy()[:, 0]
                        ...
                        all_test_ouput_sub1 = np.append(all_test_ouput_sub1, test_output_subnet1_final.data.cpu().numpy()[:, 0])
                        all_test_ouput_sub2 = np.append(all_test_ouput_sub2, test_output_subnet2_final.data.cpu().numpy()[:, 0])

                        # all_test_output = np.append(all_test_output, test_output_sum.data.cpu().numpy()[:, 0])
                        all_scores = np.append(all_scores, scores_single.data.cpu().numpy())
                        if it % 10 == 0:
                                f = open('./result/result_ours_test.txt', mode='a')
                                f.write('\n' +'sub1_test_res:'+ str(test_output_subnet1_final[0].data.cpu().numpy()) + '-' + str(scores_single.data.cpu().numpy()[0]))
                                f.write('\n' +'sub2_test_res:'+ str(test_output_subnet2_final[0].data.cpu().numpy()) + '-' + str(scores_single.data.cpu().numpy()[0]))
                                f.write('\n' + 'avg_test_res:' + str(test_output_sum_avg[0]) + '-' + str(scores_single.data.cpu().numpy()[0]))
                                f.write('\n')
                        test_loss += loss.item()

                    test_avg_loss = test_loss / (len(dset_test) / options.batch_size)
                    rho1, p_val1 = spearmanr(all_test_ouput_sub1, all_scores)
                    rho2, p_val2 = spearmanr(all_test_ouput_sub2, all_scores)
                    rho3, p_val2 = spearmanr(all_test_output_avg, all_scores)
      ...
      with torch.no_grad():
          ...
          for it, test_data in enumerate(test_loader, 0):
                ...
                test_output_sum_subnet2 = torch.cat((vid_tensor_save_subnet2[0], vid_tensor_save_subnet2[1],
                                                     ...
                                                     vid_tensor_save_subnet2[4]), 1)
                test_output_sum_subnet2 = ttc_model(test_output_sum_subnet2)* 30
                test_output_sum_subnet2 = test_output_sum_subnet2  * diffs_single
                test_output_sum_subnet1 = ttc_for_con_model(test_output_sum_subnet1) * 30
                test_output_sum_subnet1 = test_output_sum_subnet1 * diffs_single
                ...
                all_scores = np.append(all_scores, scores_single.data.cpu().numpy())
                for i in range(len(scores_single.data.cpu().numpy())):
                    f = open('./result/test-res-FInal0.txt', mode='a')
                    # f.write('\n' + 'sub1_test_res(tt-score):' + str(test_output_sum_subnet1[i].data.cpu().numpy()) + '-' + str(
                    #     scores_single.data.cpu().numpy()[i]))
                    # f.write('\n' + 'sub2_test_res(tt-score):' + str(test_output_sum_subnet2[i].data.cpu().numpy()) + '-' + str(
                    #     scores_single.data.cpu().numpy()[i]))
